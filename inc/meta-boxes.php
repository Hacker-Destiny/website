...
$meta_box = array(
    ...
    'title' => esc_html__('Page Settings', 'julia'),
    'page' => 'page',
    'context' => 'normal',
    'priority' => 'high',
    'fields' => array(
        ...
        array(
            'name' => esc_html__('Choose Page Sub Header Section', 'julia'),
            'id' => 'select_page_subheader',
            'type' => 'select',
            'options' => array(
                array('name' => esc_html__('Page Title bar', 'julia'), 'value' => 'page_titlebar'),
                array('name' => esc_html__('Slider Shortcode', 'julia'), 'value' => 'slider'),
                array('name' => esc_html__('None', 'julia'), 'value' => 'none'),
            )
        ),
        
        array(
            'name' => esc_html__('Custom Page Title', 'julia'),
            'desc' => '',
        ...
        array(
            'name' => esc_html__('Page Title Description', 'julia'),
            'desc' => '',
        ...
        array(
            'name' => esc_html__('Upload Page Titlebar Image', 'julia'),
            'desc' => '',
          ...
          array(
            'name' => esc_html__('Slider Shortcode', 'julia'),
            'desc' => '',
...
add_action('admin_menu', 'julia_kaya_add_metabox');

// Add meta box
function julia_kaya_add_metabox() {
    global $meta_box;
    add_meta_box($meta_box['id'], $meta_box['title'], 'julia_kaya_show_meta_box_data', $meta_box['page'], $meta_box['context'], $meta_box['priority']);
}

// Callback function to show fields in meta box
function julia_kaya_show_meta_box_data() {
    ...
    foreach ($meta_box['fields'] as $field) {
        ...
        switch ($field['type']) {
            ...
            case 'checkbox':
                ...
                break;
            case 'upload_image':
                echo Talents2_kaya_image_upload_field( esc_attr($field['id']), get_post_meta($post->ID, esc_attr($field['id']), true) );  // WPCS: XSS OK
                break;
        }
        echo '</td><td>',
            '</td></tr>';
...
function Talents2_kaya_image_upload_field( $name, $value = '') {
...
add_action('save_post', 'Talents2_kaya_save_meatabox_data');
// Save data from meta box
function Talents2_kaya_save_meatabox_data($post_id) {
    global $meta_box;
