    ...
    <header id="kaya-header-content-wrapper" class="site-header"> <!-- Header Section -->
        <div class="container">
            ...
            <nav id="header-navigation"> <!-- Header Navigation -->
                ...
                if ( is_active_sidebar( 'shortlist_widget_area' ) ){
                    dynamic_sidebar( 'shortlist_widget_area' );
                }   
                       ?>
                ...
                echo '<div class="toggle_search_wrapper">';
                    echo '<div class="toggle_search_field">';
                        echo '<span class="search_close">'; 
                            ...
                            echo '</i>';
                        echo '</span>';
                        // echo '<div class="overlay_toggle_search_icon">';
                          //      echo '<i class="fa fa-search"></i>';
                      //   echo '</div>'; 
...
---------------------------------------------------------------------------------------*/
function julia_kaya_footer(){ ?>
    <footer id="kaya-footer-content-wrapper" class="site-footer">
        <div class="container">
            <div class="fullwidth"><span class="copyright"><?php echo get_theme_mod('footer_copy_rights') ? get_theme_mod('footer_copy_rights') : esc_html__(' All Rights Reserved kayapati', 'julia'); // WPCS: XSS OK ?></span></div>
        </div><!-- .site-info -->
    </footer><!-- #colophon -->
    <?php
}
...
All pages, posts, archives and category page title bar
---------------------------------------------------------------------------------------*/
if(!function_exists('julia_kaya_page_title')){
    function julia_kaya_page_title(){
        $post_id = get_the_ID();
            ...
            echo '<section class="kaya-page-titlebar-wrapper" '.wp_kses($bg_image, true).' >';
                echo '<div class="container" style="">';
                    if(is_page()){
                        ...
                        echo '<h2 class="page-title">'.esc_html__( 'Posts', 'julia' ).'</h2>';

                    }elseif( is_single()){
                        echo '<h2 class="page-title">'.esc_attr($page_title).'</h2>';
                    }  elseif(is_tag()){
                        echo '<h2 class="page-title">'.esc_html__( 'Tag Archives - ', 'julia' ). single_cat_title( '', false ).'</h2>';
                    }
                    elseif ( is_author() ) {
                        the_post();
                        echo '<h2 class="page-title">'. esc_html__( 'Author Archives:', 'julia' ). ' <span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span></h2>';
                        rewind_posts();

                    } elseif (is_category()) {
                        echo '<h2 class="page-title">'. esc_html__( 'Category Archives: ', 'julia' ) . single_cat_title( '', false ).'</h2>';
                    } elseif( is_tax() ){
                        global $post;
                        $term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
                        echo '<h2 class="page-title">' .esc_attr($term->name).'</h2>';
                    }elseif (is_search()) { ?>
                        <?php echo '<h2 class="page-title">'.esc_html__( 'Search Results for:', 'julia' ).' '. get_search_query().'</h2>';
                    }elseif (is_404()) {
                        echo '<h2  class="page-title">'.esc_html__( 'Error 404 - Not Found', 'julia' ).'</h2>';
                    }elseif ( is_day() ){
                        echo '<h2 class="page-title">'.esc_html__( 'Daily Archives:', 'julia' ), '<span>' . get_the_date() . '</span></h2>';
                    }            
                    elseif ( is_month() ) { 
                        echo '<h2 class="page-title">'. esc_html__( 'Monthly Archives:', 'julia' ), '<span>' . get_the_date( 'F Y' ) . '</span></h2>';
                    } elseif ( is_year() ){
                        echo '<h2 class="page-title">'. esc_html__( 'Yearly Archives:', 'julia' ), '<span>' . get_the_date( 'Y' ) . '</span></h2>';
                    }else{ }
...
---------------------------------------------------------------------------------------*/
function julia_kaya_related_post($related_title='', $count='3', $args = array() ) {
    ...
    if ( $args['return'] == 'query' ) {
        ...
        <div class="related_posts">
            <?php echo '<h3>'.(!empty($related_title) ? esc_attr($related_title) : esc_html__('Related Posts', 'julia' )).'</h3>'; ?>
            <ul>
...
---------------------------------------------------------------------------------------*/
function julia_kaya_breadcrumb() {
    echo '<a href="'.esc_url(home_url()).'" rel="nofollow">'.esc_html__('Home', 'julia').'</a>';
...
/*---------------------------------------------------------------------------------------
 Image resizer Functionality
---------------------------------------------------------------------------------------*/
if( !function_exists('julia_kaya_image_sizes') ){
	function julia_kaya_image_sizes($url, $width, $height=0, $align='') {
 ...
 * @param int limit
 ---------------------------------------------------------------------------------------*/
if( !function_exists('julia_kaya_content_display_words') ){
  function julia_kaya_content_display_words($limit) {
...
---------------------------------------------------------------------------------------*/
if( !function_exists('julia_kaya_slider_shortcode') ){
	function julia_kaya_slider_shortcode() {
		$post_id = get_the_ID();
 ...
 * @param (float) opacity
 ---------------------------------------------------------------------------------------*/
function julia_kaya_rgba_color( $color, $opacity='0.5' ) {
    if ( $color[0] == '#' ) {
...
function julia_kaya_pagination() {
